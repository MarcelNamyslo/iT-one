...
# 📌 6. Heatmap-Daten vorbereiten (Punkte + Wert für Farbintensität)
heat_data = list(zip(all_points_gdf["Longitude"], all_points_gdf["Latitude"], all_points_gdf["Precipitation"]))
print(heat_data[:5])
...
# 📌 13. Karte speichern
m.save("districts_heatmap_2023.html")
print("✅ Karte gespeichert: districts_heatmap.html")
"""
# 📌 4. Farbcodierung für Niederschlag als PNG-Bild speichern (Speicherort anpassen!)
output_image_path = "rainfall_rendered.png"
plt.imsave(output_image_path, img_array, cmap="YlGnBu", format="png")

# 📌 5. Farblegende erstellen
colormap = cm.linear.YlGnBu_09.scale(np.nanmin(img_array), np.nanmax(img_array))
colormap.caption = "Precipitation (mm)"
...
# 📌 7. ImageOverlay mit dem Dateipfad verwenden
image = ImageOverlay(
    output_image_path,  # ✅ Nutzt den Dateipfad statt PIL.Image
    bounds=[[bounds.bottom, bounds.left], [bounds.top, bounds.right]],
    opacity=0.6
)
image.add_to(m)

# 📌 Fix: Alle Datumsspalten in Strings umwandeln
date_columns = ["date", "validOn", "validTo"]  # Falls du noch andere Datumsspalten hast, füge sie hier hinzu
for col in date_columns:
    if col in admin_gdf.columns:
        admin_gdf[col] = admin_gdf[col].astype(str)  # Konvertiere Datum zu String


# 📌 8. Bezirksgrenzen hinzufügen (hover zeigt Mean Precipitation)
folium.GeoJson(
    admin_gdf,
    name="Districts",
    style_function=lambda feature: {
    ...
    tooltip=GeoJsonTooltip(fields=["ADM3_EN", "Precipitation"], aliases=["District:", "Mean Precipitation:"])
).add_to(m)

# 📌 9. Farblegende zur Karte hinzufügen
colormap.add_to(m)

# 📌 10. Karte speichern
m.save("rainfall_with_boundaries_fixed.html")
print("✅ Karte gespeichert: rainfall_with_boundaries_fixed.html")
"""